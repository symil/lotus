...
use crate::{Rect, RenderOutput, View};

pub struct SimpleLayout<U, R, E, D> {
    rect: Rect,
    target: Rect,
    last: Option<Rect>,
    outer_margin: f64,
    ...
    dy: f64,
    dynamic: bool,
    fixed_items: Vec<(Rc<dyn View<U, R, E, D>>, Rect)>,
    movable_items: Vec<(Rc<dyn View<U, R, E, D>>, Rect)>,
}

impl<U, R, E, D> SimpleLayout<U, R, E, D> {
    ...
    pub fn push<V : View<U, R, E, D> + 'static>(mut self, view: V) -> Self {
    ...
    pub fn load(mut self, output: &mut RenderOutput<U, R, E, D>) {
        self.flush();

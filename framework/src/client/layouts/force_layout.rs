...
#[derive(Debug)]
pub struct ForceLayout<U, R, E, D> {
    pub view: Option<Rc<dyn View<U, R, E, D>>>,
    pub layout_type: ForceLayoutType,
    pub force: f64,
    pub inner_margin: f64,
    pub outer_margin: f64,
    pub scale: f64,
    pub aspect_ratio: Option<f64>,
    pub children: Vec<ForceLayout<U, R, E, D>>
}

impl<U, R, E, D> Default for ForceLayout<U, R, E, D> {
    fn default() -> Self {
        Self {
            view: None,
            layout_type: ForceLayoutType::Cell,
            force: 1.,
            inner_margin: 0.,
            outer_margin: 0.,
            scale: 1.,
...
impl<U, R, E, D> ForceLayout<U, R, E, D> {
    pub fn apply(&mut self, input: Rect) -> Vec<(Rc<dyn View<U, R, E, D>>, Rect)> {
        ...
        let is_horizontal = match self.layout_type {
            ForceLayoutType::Cell => return result,
            ForceLayoutType::Line => true,
            ForceLayoutType::Column => false,
        };
    ...
    pub fn cell(&mut self) {
        self.layout_type = ForceLayoutType::Cell;
    }

    pub fn line(&mut self) {
        self.layout_type = ForceLayoutType::Line;
    }

    pub fn column(&mut self) {
        self.layout_type = ForceLayoutType::Column;
    }
}

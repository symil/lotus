...
impl VarPathSegment {
    pub fn process(&self, parent_type: &Type, access_type: AccessType, context: &mut ProgramContext) -> Option<Wasm> {
        match self {
            ...
            VarPathSegment::BracketIndexing(index_expr) => process_bracket_indexing(parent_type, index_expr, access_type, context),
        }
    }
}


pub fn process_bracket_indexing(parent_type: &Type, index_expr: &Expression, access_type: AccessType, context: &mut ProgramContext) -> Option<Wasm> {
    let mut result = None;
    ...
    if let Type::Array(item_type) = parent_type {
        let func_name = match access_type {
            AccessType::Get => item_type.get_array_get_function_name(),
            AccessType::Set(_) => item_type.get_array_set_function_name(),
        };

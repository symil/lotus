...
impl CompilerConstant {
    pub fn process_as_value(&self, context: &mut ProgramContext) -> Option<ValueOrType> {
        match context.get_current_type() {
                ...
                type_wrapped.with_ref(|type_unwrapped| {
                    match self.name.as_str() {
                        ...
                        COMPILER_CONSTANT_FIELD_NAME => {
                            context.errors.add(self, format!("compiler constant `{}` cannot be processed as a value", format!("#{}", &self.name).bold()));
                            None
                        },
                        COMPILER_CONSTANT_FIELD_TYPE => match context.iter_fields_counter {
                            ...
                            None => {
                                context.errors.add(self, format!("compiler constant `{}` can only be accessed from within an `iter_fields` block", format!("#{}", &self.name).bold()));
                                None
                            },
                        },
                        _ => {
                            context.errors.add(self, format!("compiler constant `{}` does not exist", format!("#{}", &self.name).bold()));
                            None
                        }
                    }
                })
            ...
            None => {
                context.errors.add(self, format!("compiler constant `{}` can only be accessed from within a method", format!("#{}", &self.name).bold()));
                None
            },
        }
    }
    ...
    pub fn process_as_name(&self, context: &mut ProgramContext) -> Option<Identifier> {
        match self.name.as_str() {
            ...
            COMPILER_CONSTANT_CLASS_ID | COMPILER_CONSTANT_FIELD_COUNT | COMPILER_CONSTANT_FIELD_TYPE =>  {
                context.errors.add(self, format!("compiler constant `{}` cannot be processed as a name", format!("#{}", &self.name).bold()));
                None
            },
            _ => {
                context.errors.add(self, format!("compiler constant `{}` does not exist", format!("#{}", &self.name).bold()));

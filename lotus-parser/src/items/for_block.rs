...
impl ForBlock {
    pub fn process(&self, context: &mut ProgramContext) -> Option<IrFragment> {
        ...
        if let Some(range_end) = &self.range_end {
            if let (Some(range_start_wasm), Some(range_end_wasm)) = (range_start_wasm_opt, range_end_wasm_opt) {
                if !range_start_wasm.ty.is_integer() {
                    context.errors.add(&self.range_start, format!("range start: expected `{}`, got `{}`", context.int_type(), &range_start_wasm.ty));
                    ok = false;
                }

                if !range_end_wasm.ty.is_integer() {
                    context.errors.add(range_end, format!("range end: expected `{}`, got `{}`", context.int_type(), &range_end_wasm.ty));
                    ok = false;
                }

                let range_end_var_name = Identifier::unique("range_end", self);

                let index_var_info = context.push_var(index_var_name.clone(), context.int_type(), VariableKind::Local);
                let item_var_info = context.push_var(item_var_name.clone(), context.int_type(), VariableKind::Local);
                let range_end_var_info = context.push_var(range_end_var_name.clone(), context.int_type(), VariableKind::Local);
            ...
            let item_var_info = context.push_var(&item_var_name, iterable_wasm.ty.get_item_type(), VariableKind::Local);

            let ptr_get_func_name = item_var_info.ty.pointer_get_function_name();

            if let Some(block_wasm) = self.statements.process(context) {
                content.extend(iterable_wasm.wat);
                content.extend(vec![
        ...
        match ok {
            true => Some(IrFragment::new(TypeOld::Void, content, variables)),
            false => None
        }
    }
}
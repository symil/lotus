...
impl ArrayLiteral {
    pub fn process(&self, context: &mut ProgramContext) -> Option<Vasm> {
        ...
        let mut instructions = vec![
            VI::set(&array_var, VI::call_method(&final_array_type, final_array_type.get_static_method(NEW_FUNC_NAME, context).unwrap(), &[], vec![VI::int(self.items.len())])),
            VI::set(&array_body_var, VI::call_method(&final_array_type, final_array_type.get_regular_method(GET_BODY_FUNC_NAME, context).unwrap(), &[], vec![VI::get(&array_var)])),
        ];

        for (i, item_vasm) in item_vasm_list.into_iter().enumerate() {
            instructions.extend(vec![
                VI::get(&array_body_var),
                VI::call_method(&final_pointer_type, final_pointer_type.get_regular_method(SET_AT_INDEX_FUNC_NAME, context).unwrap(), &[], vasm![VI::int(i), item_vasm])
            ]);
        }

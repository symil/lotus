...
impl StringLiteral {
    ...
    pub fn process(&self, context: &mut ProgramContext) -> Option<Vasm> {
        ...
        let mut content = vec![
            VI::call_function(string_type.get_static_method(NEW_FUNC_NAME).unwrap(), vec![VI::int(unescaped_chars.len())])
        ];

        for (i, code) in unescaped_chars.into_iter().enumerate() {
            content.push(
                VI::call_function(string_type.get_method(SET_CHAR_FUNC_NAME).unwrap(), vec![VI::int(i), VI::int(code)]),
            );
        }

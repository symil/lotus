use crate::{generation::{LOG_I32_FUNC_NAME, MEMORY_ALLOC_FUNC_NAME, MEMORY_FREE_FUNC_NAME, MEM_ALLOC_FUNC_NAME, MEM_FREE_FUNC_NAME, ToWat, ToWatVec, VALUE_BYTE_SIZE, Wat}, items::Identifier, wat};
...
pub fn process_system_method_call(method_name: &Identifier, context: &mut ProgramContext) -> Option<Wasm> {
    let (arguments, return_type, wat) = match method_name.as_str() {
        ...
        "log_ptr" => (vec![Type::int_pointer()], Type::Void, Wat::call_from_stack(LOG_I32_FUNC_NAME)),
        _ => return None
    };

...
pub fn process_string_field_access(field_name: &Identifier, context: &mut ProgramContext) -> Option<IrFragment> {
    match field_name {
        _ => None
    }
}

pub fn process_string_method_call(method_name: &Identifier, context: &mut ProgramContext) -> Option<IrFragment> {
    let (arguments, return_type, func_name) = match method_name.as_str() {
        "len" => (vec![], TypeOld::Integer, STRING_GET_LENGTH_FUNC_NAME),
        _ => return None
    };

    Some(IrFragment::simple(TypeOld::Function(arguments, Box::new(return_type)), Wat::call_from_stack(func_name)))
}
...
pub fn process_array_method_call(item_type: &TypeOld, method_name: &Identifier, context: &mut ProgramContext) -> Option<Wasm> {
    let (arguments, return_type, wat) = match method_name.as_str() {
        ...
        "pop" => (vec![], TypeOld::array(item_type.clone()), vec![Wat::call_from_stack(ARRAY_POP_FUNC_NAME)]),
        "push" => {
            let mut wat = vec![];
            ...
            (vec![item_type.clone()], TypeOld::array(item_type.clone()), wat)
        },
        "concat" => (vec![TypeOld::array(item_type.clone())], TypeOld::array(item_type.clone()), vec![Wat::call_from_stack(ARRAY_CONCAT_FUNC_NAME)]),
        _ => return None
    };

...
impl MacroContext {
    fn process_as_value(self, location: &DataLocation, context: &mut ProgramContext) -> Option<Vasm> {
        match self.mac {
            MacroName::Line => Some(Vasm::new(context.int_type(), vec![], vec![VI::int(location.line)])),
            _ => self.current_type.clone().unwrap().with_ref(|type_unwrapped| {
                match self.mac {
                    ...
                    MacroName::FieldDefaultExpression => Some(self.field_info.unwrap().default_value.clone()),
